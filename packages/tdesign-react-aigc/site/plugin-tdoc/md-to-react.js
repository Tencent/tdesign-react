...
export default async function mdToReact(options) {
  ...
  const reactSource = `
    ...
    import Stackblitz from '@tdesign/react-aigc-site/src/components/stackblitz/index.jsx';
    import Codesandbox from '@tdesign/react-aigc-site/src/components/codesandbox/index.jsx';
    ${demoDefsStr}
    ${demoCodesDefsStr}
...
async function customRender({ source, file, md }) {
  ...
  // md filename
  const reg = file.match(/packages\/pro-components\/chat\/(\w+-?\w+)\/(\w+-?\w+)\.?(\w+-?\w+)?\.md/);
  ...
  if (pageData.usage && pageData.isComponent) {
    const usageObj = compileUsage({
      ...
      demoPath: path.posix.resolve(__dirname, `../../../pro-components/chat/${componentName}/_usage/index.jsx`),
    });
    if (usageObj) {
