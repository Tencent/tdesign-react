...
type TableMountProps = Partial<BaseTableProps>;
type TableComponent = ComponentType<BaseTableProps>;
type TableRerender = (ui: ReactElement) => void;
...
export function getNormalTableMount(TTable: TableComponent, props?: TableMountProps) {
  return render(
    <TTable
      rowKey="index"
      data={getTableData()}
...
export function getEmptyDataTableMount(TTable: TableComponent, props?: TableMountProps) {
  return render(<TTable rowKey="index" data={[]} columns={SIMPLE_COLUMNS} {...props}></TTable>);
}

/** 获取用于本地数据分页的表格数据（受控和非受控测试） */
export function getDataLengthLargerThanPageSizeTableMount(
  TTable: TableComponent,
  props: TableMountProps,
 ...
 * 远程数据分页
 */
export function getNormalCountDataTableMount(TTable: TableComponent, props: TableMountProps, rerender?: TableRerender) {
  const renderFun = rerender || render;
  return renderFun(
