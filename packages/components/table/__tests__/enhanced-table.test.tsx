  ...
  describe('scenarios', () => {
    describe('select', () => {
      ...
      it('select-all with disabled rows', async () => {
        ...
        const { container } = render(<TEnhancedTable onSelectChange={onSelectChange} />);
      ...
      it('select-all keeps preselected disabled row', async () => {
        ...
        const { container } = render(
          <TEnhancedTable defaultSelectedRowKeys={['1-1-0']} onSelectChange={onSelectChange} />,
        );
      ...
      it('select parent[checkStrictly]', async () => {
        const onLooseSelectChange = vi.fn();
        const { container: looseContainer } = render(
          <TEnhancedTable defaultSelectedRowKeys={['1-1-0']} onSelectChange={onLooseSelectChange} />,
        );

        const looseRows = looseContainer.querySelectorAll('tr.t-table-tr--level-0');
        const looseParentCheckbox = looseRows[1].querySelector('.t-checkbox');

        expect(looseParentCheckbox).toHaveClass('t-is-indeterminate');

        await fireEvent.click(looseParentCheckbox);

        const selectedKeys = onLooseSelectChange.mock.calls[0][0];

        expect(looseParentCheckbox).toHaveClass('t-is-checked');
        expect(selectedKeys).toContain('1');
        expect(selectedKeys).toContain('1-1-0');
        expect(selectedKeys).toHaveLength(13);

        onLooseSelectChange.mockClear();
        await fireEvent.click(looseParentCheckbox);

        expect(onLooseSelectChange).toHaveBeenCalledWith(['1-1-0'], expect.any(Object));

        const onStrictSelectChange = vi.fn();
        const { container: strictContainer } = render(
          <TEnhancedTable defaultSelectedRowKeys={['1-1-0']} onSelectChange={onStrictSelectChange} checkStrictly />,
        );
        const strictRows = strictContainer.querySelectorAll('tr.t-table-tr--level-0');

        const strictParentCheckbox = strictRows[1].querySelector('.t-checkbox');
        expect(strictParentCheckbox).not.toHaveClass('t-is-indeterminate');
        expect(strictParentCheckbox).not.toHaveClass('t-is-checked');

        await fireEvent.click(strictParentCheckbox);
        expect(onStrictSelectChange).toHaveBeenCalledWith(expect.arrayContaining(['1-1-0', '1']), expect.any(Object));

        const childCheckbox = strictRows[2].querySelector('.t-checkbox');
        onStrictSelectChange.mockClear();
        await fireEvent.click(childCheckbox);
        expect(onStrictSelectChange).toHaveBeenCalledWith(
          expect.arrayContaining(['1-1-0', '1', '2']),
          expect.any(Object),
        );
      });
    });

    it('shows children immediately after expanding a node', async () => {
      const VIRTUAL_ROW_HEIGHT = 40;
      const TABLE_HEIGHT = 400;
      const ROOT_COUNT = 150;
      const BUFFER_SIZE = 5;

      function getVirtualTreeData(rootCount = 150, childrenPerRoot = 3) {
