...
const Breadcrumb = forwardRefWithStatics(
  (props: BreadcrumbProps, ref: React.Ref<HTMLDivElement>) => {
    ...
    const content = useMemo(() => {
      ...
      if (
        maxItems &&
        (itemsAfterCollapse || 0) + (itemsBeforeCollapse || 0) >= maxItems
      ) {
        log.error('Breadcrumb', `maxItems={${maxItems}} 必须大于 itemsBeforeCollapse={${itemsBeforeCollapse}} + itemsAfterCollapse={${itemsAfterCollapse}}`);
        return items;
      }
