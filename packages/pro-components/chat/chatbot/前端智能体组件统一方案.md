...
为了解决非交互式场景下 handler 与 component 分离的问题，我们引入一个统一的 Action 配置对象 (AgentActionConfig)。一个 Action 的所有相关逻辑——包括元数据、业务逻辑 (handler) 和 UI 逻辑 (component)——都封装在这个单一的对象中，实现了高度内聚。
适配器会根据此配置对象中是否存在 handler 属性，来智能地决定其行为模式。

## 3. 技术规范

### 3.1. 组件 Props 接口：AgentComponentProps
...
### 3.2. Action 配置接口：AgentActionConfig

这是方案的核心，它通过 TypeScript 的联合类型定义了两种 Action 模式。

```
import { AgentComponentProps } from './agent-spec';
...
interface NonInteractiveActionConfig<TArgs, TResult> {
  ...
  /** 状态显示组件 */
  component: React.FC<AgentComponentProps<TArgs, TResult>>;
}
...
// 最终的配置类型
export type AgentActionConfig<TArgs, TResult, TResponse> =
...
import { AgentActionConfig } from './agent-config-spec';
import { AgentComponentProps } from './agent-spec';
// 类型守卫：判断是否为非交互式配置
 ...
 */
export function useAgentAction<TArgs, TResult, TResponse>(
  ...
  if (isNonInteractive<TArgs, TResult>(config)) {
    ...
    useCopilotAction<any>({
      ...
      render: (props) => {
        const agentProps: AgentComponentProps<TArgs, TResult> = {
          status: props.status,
    ...
    useCopilotAction<any>({
      ...
      renderAndWaitForResponse: (props) => {
        const agentProps: AgentComponentProps<TArgs, TResult, TResponse> = {
          status: props.status,
          args: props.args as TArgs,
          result: props.result,
          error: props.error,
...
export const ReportGeneratorStatus: React.FC<AgentComponentProps<ReportArgs, ReportResult>> = ({
  status,
  args,
  result,
  error
...
import { AgentActionConfig } from '../specs';
import { ReportGeneratorStatus } from '../components/ReportGeneratorStatus';
export const generateReportAction: AgentActionConfig = {
  name: "generateReport",
...
// 实现组件
export const ConfirmDeletionDialog: React.FC<AgentComponentProps<ConfirmArgs, ConfirmResult, ConfirmResponse>> = ({
  status,
  args,
  result,
  respond
...
export const deleteItemAction: AgentActionConfig = {
  name: "deleteItem",
...
function MyDataGrid() {
  // 调用方式与非交互式完全一致
  useAgentAction(deleteItemAction);
  return (/* UI to trigger the action */);
}
```
...
// 实现组件，包含内部状态管理
export const FileUpload: React.FC<AgentComponentProps<UploadArgs, UploadResult, UploadResponse>> = ({
  status,
  args,
  result,
  respond
...
export const uploadFileAction: AgentActionConfig = {
  name: "uploadFile",
...
function MyUploader() {
  useAgentAction(uploadFileAction);
  return (/* UI to trigger the action */);
}
```
...
## 6. 方案优势
     ...
     高度内聚 (High Cohesion): 一个 Action 的所有逻辑（业务 handler 和 UI component）都封装在同一个配置对象中，代码组织清晰，维护性极高。
             2.
     API 简化 (Simplified API): 开发者只需学习和使用一个统一的 useAgentAction Hook，无需关心底层使用了哪种 CopilotKit 模式，心智负担大大降低。
             3.
