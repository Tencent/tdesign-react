...
| onChatSent        | Function        | -      | 发送消息回调，TS 类型：`(e: CustomEvent<ChatRequestParams>) => void`                                                                                                                                                                                                         | N    |
...
| onMessage  | Function | -      | 处理流式消息的回调。TS 类型：`(chunk: SSEChunkData) => AIMessageContent / null`              | N    |
| onComplete | Function | -      | 请求结束时的回调。TS 类型：`(isAborted: boolean, params: RequestInit, result?: any) => void` | N    |
