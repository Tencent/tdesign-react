...
import {
  ...
  isUserMessage,
  AgentStateProvider,
  createToolConfigWithState,
} from '../index';
import type { ToolcallComponentProps } from '../components/toolcall/types';
 ...
 */
export const VideoClipSteps: React.FC<VideoClipStepsProps> = ({ clipState, stateKey }) => {
  // 订阅AG-UI状态事件
...
export default function VideoClipAgentChatWithSubscription() {
  ...
  useAgentToolcall(
    createToolConfigWithState({
      ...
      component: ({ status, args, error, agentState }: ToolcallComponentProps<ShowStepsArgs>) => {
        if (status === 'error') {
        ...
        return <VideoClipSteps clipState={agentState?.[stateKey]} stateKey={stateKey} />;
      },
    }),
  );
