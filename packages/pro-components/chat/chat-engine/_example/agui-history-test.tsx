...
import { AGUIAdapter } from '../core/adapters/agui';
import type { AGUIHistoryMessage, AGUIActivityMessage } from '../core/adapters/agui';
  ...
  const runTest = () => {
    try {
      ...
      const analysis = {
        ...
        customCount: mockAGUIHistory.filter(msg => 
          msg.role === 'activity' && (msg as any).activityType === 'CUSTOM'
        ).length,
        errors: [] as string[],
      };
          ...
          msg.content.forEach((content: any, contentIndex: number) => {
            ...
            if (content.type?.startsWith('activity-')) {
              if (!content.data?.activityType || content.data?.content === undefined) {
                analysis.errors.push(
                  `消息 ${index} 内容 ${contentIndex}: Activity 数据结构不完整`
                );
              }
            }
            // 检查 custom 内容是否正确转换
            if (content.type === 'custom') {
              if (!content.data?.name) {
                analysis.errors.push(
                  `消息 ${index} 内容 ${contentIndex}: Custom 数据结构不完整`
                );
              }
            }
          });
    ...
    return (
      <Card key={index} size="small" style={{ marginBottom: 8 }}>
        ...
        <div style={{ fontSize: 14 }}>
          {msg.role === 'user' && (
            <div><strong>Content:</strong> {(msg as any).content}</div>
          )}
          {msg.role === 'assistant' && (
            <div><strong>Content:</strong> {(msg as any).content || '(无文本内容)'}</div>
          )}
          {msg.role === 'activity' && (
            <div>
              <strong>Content:</strong>
              <pre style={{ fontSize: 11, background: '#f5f5f5', padding: 8, borderRadius: 4, marginTop: 4, maxHeight: 200, overflow: 'auto' }}>
          ...
          {Array.isArray(msg.content) ? (
            <div>
              ...
              {msg.content.map((content: any, contentIndex: number) => (
                <div key={contentIndex} style={{ marginLeft: 16, marginTop: 8, padding: 8, background: '#f9f9f9', borderRadius: 4 }}>
                  <div style={{ fontSize: 12, color: '#666', marginBottom: 4 }}>
                  ...
                  {content.type?.startsWith('activity-') && (
                    <div>
                      ...
                      <pre style={{ fontSize: 10, background: '#fff', padding: 4, borderRadius: 2, marginTop: 4, maxHeight: 150, overflow: 'auto' }}>
            ...
            <div><strong>Content:</strong> {JSON.stringify(msg.content)}</div>
          )}
        </div>
      </Card>
    );
            ...
            <>
              ...
              <Card title="测试结果摘要">
                <Space direction="vertical">
                  <div>原始消息数量: <strong>{testResult.analysis.totalOriginal}</strong></div>
                  <div>转换后消息数量: <strong>{testResult.analysis.totalConverted}</strong></div>
                  <div>Activity 消息数量: <strong>{testResult.analysis.activityCount}</strong></div>
                  <div>Custom 消息数量: <strong>{testResult.analysis.customCount}</strong></div>
              ...
              <div style={{ display: 'flex', gap: 16 }}>
                {/* 原始消息 */}
                <div style={{ flex: 1 }}>
                  <Card title={`原始 AG-UI 消息 (${testResult.original.length} 条)`} style={{ height: 700, overflow: 'auto' }}>
                ...
                {/* 转换后消息 */}
                <div style={{ flex: 1 }}>
                  <Card title={`转换后的 ChatEngine 消息 (${testResult.converted.length} 条)`} style={{ height: 700, overflow: 'auto' }}>
