/**
 * AG-UI + json-render 集成示例
 *
 * 演示内容：
 * 1. 使用 AG-UI 协议（protocol: 'agui'）接收流式数据
 * 2. 通过 ACTIVITY_SNAPSHOT/ACTIVITY_DELTA 事件传递 json-render Schema
 * 3. 使用 useAgentActivity 注册 json-render 渲染器
...
export default function AguiJsonRenderExample() {
  const [inputValue, setInputValue] = useState('使用 json-render 创建一个用户信息表单');
  ...
  return (
    <div style={{ height: '800px', display: 'flex', flexDirection: 'column' }}>
      ...
      <ChatSender
        value={inputValue}
        placeholder="输入消息，例如：创建用户信息表单 / 创建用户信息卡片"
