...
export function createRegistryManager<TConfig extends BaseConfig, TProps>(
  ...
  const renderFunctionCache = new Map<string, React.MemoExoticComponent<React.ComponentType<TProps>>>();
  ...
  return {
    ...
    getRenderFunction(key: string): React.MemoExoticComponent<React.ComponentType<TProps>> | null {
      ...
      let memoizedComponent = renderFunctionCache.get(key);

      if (!memoizedComponent) {
        // 创建 memo 化的组件
        memoizedComponent = React.memo((props: TProps) => React.createElement(config.component, props));

        // 缓存组件
        renderFunctionCache.set(key, memoizedComponent);
      }
