...
虽然可以通过[自定义渲染内容](#自定义渲染)的方式（扩展类型 + 插槽）来实现工具调用，但这种方式存在以下问题：

**传统方式的问题：**

1. **逻辑分散**：业务逻辑（handler）和 UI 组件（component）分离在不同位置，代码组织混乱，维护成本高
2. **重复劳动**：每个工具都需要重复编写类型扩展、数据解析、插槽渲染等样板代码
3. **缺乏规范**：没有统一的组件接口规范，不同开发者实现方式各异，难以复用和协作
4. **状态管理复杂**：需要手动处理工具调用的各种状态

**useAgentToolcall + ToolCallRenderer 的优势：**

基于`useAgentToolcall` Hook 配合 `ToolCallRenderer` 组件提供了完整的工具调用解决方案：

- **高度内聚**：一个工具配置对象封装了元数据、业务逻辑和 UI 组件，代码组织清晰
- **API 简化**：统一的 Hook 接口和标准的 Props 契约（`ToolcallComponentProps`），降低学习成本
- **自动化渲染**：`ToolCallRenderer` 自动处理配置查找、参数解析、状态管理，无需手动编写渲染逻辑
- **可移植性**：独立的配置对象易于管理、动态加载，可轻松移植到其他框架
- **组件生态**：基于配置化组件动态注册的机制，为后续实现统一的工具调用共享组件生态提供可能性

**工作流程：**

1. **注册阶段**：使用 `useAgentToolcall` 注册工具配置到全局注册表
2. **渲染阶段**：使用 `ToolCallRenderer` 组件，它会自动：
   - 根据 `toolCallName` 查找配置
   - 解析 `args` 参数
   - 随着后端`TOOL_CALL_*`事件的过程管理状态流转（start → args → content → end）
   - 渲染配置中的 `component`
   - 执行 `handler`（如果是非交互式工具）
...
// ✅ useAgentToolcall + ToolCallRenderer：高度内聚，自动化渲染
// 1. 注册工具配置
useAgentToolcall([{
  name: 'get_weather',
