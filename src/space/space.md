...
size | String / Number | medium | 间距大小。可选项：small/medium/large。TS 类型：`Size | Size[]
` `type Size = number | SizeEnum`。[通用类型定义](https://github.com/Tencent/tdesign-react/blob/develop/src/common.ts)。[详细类型定义](https://github.com/Tencent/tdesign-react/blob/develop/src/space/type.ts) | N
