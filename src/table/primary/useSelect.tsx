...
function useSelect(props: PrimaryTableProps): [PrimaryTableCol[]] {
  ...
  const { checkStrictly, childrenKey = 'children', getFlattenRowData, getFlattenPageData } = useEnhancedTableContext();
  const isControlled = !!selectedRowKeys;
  const [innerSelectedRowKeys, setInnerSelectedRowKeys] = useState(defaultSelectedRowKeys || []);
  const selectColumn = useMemo(() => columns.find((column: PrimaryTableCol) => !!TypeEnum[column.type]), [columns]);
  const flattenRowData = getFlattenRowData?.(!!selectColumn) || data;
  const transformedColumns = useMemo(() => {
    const disabledFn = getDisableFn({ selectColumn });

    function getSelectColumns(columns, innerSelectedRowKeys) {
      return columns.map((column: PrimaryTableCol) => {
        ...
        if (isMultiple) {
          ...
          cell = (cellParams: CellParams) => {
            ...
            return (
              <Radio checked={checked} disabled={disabled} onChange={() => onRadioChange(currentRowKeyValue, row)} />
            );
          };
        }
    ...
    function onTitleCheckboxChange({ flattenPageData, checked }) {
      ...
      if (checked) {
        ...
        flattenPageData.forEach((rowItem, index) => {
          ...
          const isChecked = innerSelectedRowKeys.includes(currentRowKeyValue);
          if (!isDisabled && isChecked) {
            const rowKeyIndex = innerSelectedRowKeys.findIndex(
              (selectedRowKey) => selectedRowKey === currentRowKeyValue,
            );
      ...
      onSelectChangeRef.current?.(selectedRowKeysNew, selectOptions);
    }
      ...
      if (childrenRowKeys.length) {
        if (checked) {
          ...
          childrenRowKeys.forEach((childrenRowKey) => {
            const childrenRowKeyIndex = selectedRowKeys.findIndex(
              (selectedRowKey) => selectedRowKey === childrenRowKey,
            );
    ...
    return getSelectColumns(columns, innerSelectedRowKeys);
  }, [
    checkStrictly,
    childrenKey,
    classPrefix,
