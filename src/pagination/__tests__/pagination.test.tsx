...
describe('Pagination test', () => {
  ...
  test('jumper', () => {
    ...
    fireEvent.change(document.querySelector('.t-pagination__jump input'), { target: { value: '5' } });
    fireEvent.keyUp(document.querySelector('.t-pagination__jump input'), { keyCode: 13 });
    expect(document.querySelector('.t-is-current')).toHaveTextContent('5');

    fireEvent.change(document.querySelector('.t-pagination__jump input'), { target: { value: '30' } });
    fireEvent.keyUp(document.querySelector('.t-pagination__jump input'), { keyCode: 13 });
    expect(document.querySelector('.t-is-current')).toHaveTextContent('20');

    fireEvent.change(document.querySelector('.t-pagination__jump input'), { target: { value: '-1' } });
    fireEvent.keyUp(document.querySelector('.t-pagination__jump input'), { keyCode: 13 });
