...
onRecentColorsChange | Function |  | TS 类型：`(value: Array<string>, context: { trigger: RecentColorsChangeTrigger }) => void`<br/>最近使用颜色发生变化时触发。第一个参数 `value` 表示变化后的色值，`context.trigger` 表示触发颜色变化的来源。[详细类型定义](https://github.com/Tencent/tdesign-react/blob/develop/src/color-picker/type.ts)。<br/>`type RecentColorsChangeTrigger = 'delete' | 'clear' | 'input' | 'palette-saturation-brightness'  | 'palette-saturation' | 'palette-brightness' | 'palette-hue-bar' | 'palette-alpha-bar'`<br/> | N
