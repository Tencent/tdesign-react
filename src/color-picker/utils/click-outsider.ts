...
type NodeElement = HTMLElement | ReactNode;

const createDocumentHandler =
  (elements: HTMLElement[], handler: Handler, includePopup = true, selector: string) =>
  (e: MouseEvent) => {
    if (includePopup) {
      document.querySelectorAll(selector).forEach((ele: Element) => {
...
export const useClickOutsider = () => {
  ...
  const addClickOutsider = (els: NodeElement[], handler: Handler) => {
    ...
    const documentHandler = createDocumentHandler(elements, handler, true, POPUP_SELECTOR);
