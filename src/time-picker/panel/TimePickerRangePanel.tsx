...
const TimePickerPanel: FC<TimeRangePickerPanelProps> = (props) => {
  ...
  const handlePanelValueChange = (value: string, index: number) => {
    index === 0 ? onChange([value, endTime]) : onChange([startTime, value] as any);
  };

  return (
    <div className={classNames(panelClassName, `${panelClassName}-section`)}>
      <div className={`${panelClassName}-section__body`}>
        <SinglePanel {...props} value={startTime} onChange={(v) => handlePanelValueChange(v, 0)} />
        <SinglePanel {...props} value={endTime} onChange={(v) => handlePanelValueChange(v, 1)} />
      </div>
      {isFooterDisplay ? (
