...
keys | Object | - | 用来定义选项文本和选项值字段，示例：{ label: 'text', value: 'id' }，表示选项文本取 `text` 字段，选项值取 `id` 字段。TS 类型：`KeysType`。[详细类型定义](https://github.com/TDesignOteam/tdesign-react/blob/main/src/transfer/type.ts) | N
...
onChange | Function |  | 数据列表发生变化时触发。TS 类型：`TNode`。[通用类型定义](https://github.com/TDesignOteam/tdesign-react/blob/main/src/common.ts)。[详细类型定义](https://github.com/TDesignOteam/tdesign-react/blob/main/src/transfer/type.ts)。`(targetValue: Array<TransferValue>, params: TargetParams) => {}` | N
onCheckedChange | Function |  | 源数据列表或目标数据列表的选中项发生变化时触发。TS 类型：`TNode`。[通用类型定义](https://github.com/TDesignOteam/tdesign-react/blob/main/src/common.ts)。[详细类型定义](https://github.com/TDesignOteam/tdesign-react/blob/main/src/transfer/type.ts)。`(options: CheckedOptions) => {}` | N
