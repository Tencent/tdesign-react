...
onSubmit | Function |  | 表单提交时触发。其中 validateResult 表示校验结果，firstError 表示校验不通过的第一个规则提醒。[详细类型定义](https://github.com/TDesignOteam/tdesign-react/blob/main/src/form/type.ts)。`(context: SubmitContext<FormData>) => {}` | N
