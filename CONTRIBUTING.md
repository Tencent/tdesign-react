...
└── packages                  // 子应用
    ...
    │   ├── style
    │   ├── utils
    └── tdesign-react      // tdesign-react 出口
        └── site              // 站点入口


### 组件页路由配置

每一个组件都有自己的路由，页面配置都是一个 `Markdown` 文件，如`button.md`，具体路径可参考 `/packages/tdesign-react/site/site.config.mjs`。如果有新增组件，直接按照模板添加即可
  ...
  children: [
    {
      ...
      name: 'button',
      path: '/react/button',
      component: () => import(`@/packages/tdesign-react/button/button.md`),
    },
    {
      title: 'Icon 图标',
      name: 'icon',
      path: '/react/icon',
      component: () => import(`@/packages/tdesign-react/icon/icon.md`),
    },
    ...
  ],
},
...
[组件测试文档](./packages/tdesign-react/test/README.md)
...
`UI` 是多个框架共用的，比如 `PC` 端的 `Vue、React` 都是复用子仓库的 `UI` 代码。
